  ...
  (define make-transcoder
    (lambda (codec . options)
      ...
      (let-optionals options ((eol-style (native-eol-style)) (error-handling-mode 'replace))
        (let ((octet0 (and (eq? (tuple-ref codec 0) 'type:codec) (port-lookup-codec-code (tuple-ref codec 1))))
              (octet1 (port-lookup-eol-style-code eol-style))
              (octet2 (port-lookup-error-handling-mode-code error-handling-mode)))
          (or octet0 (assertion-violation 'make-transcoder (format "expected codec, but got ~r, as argument 1" codec)))
          (or octet1 (assertion-violation 'make-transcoder (format "expected eol-style, but got ~r, as argument 2" eol-style)))
          (or octet2 (assertion-violation 'make-transcoder (format "expected error-handling-mode, but got ~r, as argument 3" error-handling-mode)))
          (let ((bv (make-bytevector 3)))
            (bytevector-u8-set! bv 0 octet0)
            (bytevector-u8-set! bv 1 octet1)
            (bytevector-u8-set! bv 2 octet2)
