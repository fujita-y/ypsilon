...
(define re-api-entry (pregexp "^GLFWAPI\\s+([a-zA-Z0-9* ]+)\\s+((?:GLFW|glfw)[a-zA-Z0-9]+)[(](.*)[)];\\s*$"))
(define re-api-entry-continue (pregexp "^GLFWAPI\\s+.+[,]\\s*$"))
(define re-api-const (pregexp "^#define\\s+(GLFW_[A-Z0-9_]+)\\s+([^/]+)$"))
(define re-api-callback-type (pregexp "GLFW[a-zA-Z0-9]+(fun|proc)"))
...
(define convert-type
  (lambda (s)
    (cond ((string-contains s "*") "void*")
          ((pregexp-match re-api-callback-type s) "void*")
          (else
            (let loop ((type types))
              (cond ((null? type) s)
...
(define convert-logical-or
  (lambda (s)
    (cond ((string-contains s "|")
           (pregexp-replace "[(]" (pregexp-replace* " [|] " s " ") "(+ "))
          (else s))))
                     ...
                     (let ((name (pregexp-substring line m 1)) (value (pregexp-substring line m 2)))
                       (set! acc
                             (cons (list
                                     ...
                                     (convert-logical-or (pregexp-replace "0x" (trim-whitespace value) "#x")))
                                   acc)))
                     (loop (get-line input))))
...
(define api-entry (parse-api-entry "toolbox/h/glfw3.h"))
(define api-const (parse-api-const "toolbox/h/glfw3.h"))
(define exports (append (map cadr api-entry) (map car api-const)))

(call-with-port
  (open-file-output-port "toolbox/output/glfw.scm" (file-options no-fail) (buffer-mode block) (native-transcoder))
  (lambda (port)
    ...
    (format port "(library (ypsilon glfw)~%")
    (output-exports port exports)
    (format port "  (import (core) (ypsilon c-ffi))~%")
    (format port "  (define libGLFW~%")
    (format port "    (let ((sysname (architecture-feature 'sysname)))~%")
    (format port "      (cond ((string-contains sysname \"darwin\")~%")
    (format port "             (load-shared-object \"libglfw.dylib\"))~%")
    (format port "            ((string-contains sysname \"linux\")~%")
    (format port "             (load-shared-object \"libglfw.so.3\"))~%")
    (format port "            (else~%")
    (format port "              (assertion-violation 'load-shared-object \"can not load GLFW library, unknown operating system\")))))~%")
