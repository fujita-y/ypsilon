...
scm_obj_t subr_set_port_current_line(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 2) {
    if (PORTP(argv[0])) {
...
scm_obj_t subr_set_port_current_column(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 2) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// port?
scm_obj_t subr_port_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
...
// input-port?
scm_obj_t subr_input_port_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// output-port?
scm_obj_t subr_output_port_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// port-closed?
scm_obj_t subr_port_closed_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
scm_obj_t subr_output_port_buffer_mode(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
      ...
      try {
        ...
        switch (i) {
          ...
          case SCM_PORT_BUFFER_MODE_LINE:
            return make_symbol(vm->m_heap, "line");
...
// flush-output-port
scm_obj_t subr_flush_output_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// close-port
scm_obj_t subr_close_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// eof-object
scm_obj_t subr_eof_object(VM* vm, int argc, scm_obj_t argv[]) {
...
// eof-object?
scm_obj_t subr_eof_object_pred(VM* vm, int argc, scm_obj_t argv[]) {
...
// current-input-port
scm_obj_t subr_current_input_port(VM* vm, int argc, scm_obj_t argv[]) {
...
// current-output-port
scm_obj_t subr_current_output_port(VM* vm, int argc, scm_obj_t argv[]) {
...
// current-error-port
scm_obj_t subr_current_error_port(VM* vm, int argc, scm_obj_t argv[]) {
...
// standard-input-port
scm_obj_t subr_standard_input_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 0) {
    try {
      scm_port_t port;
...
#endif
      port = make_std_port(vm->m_heap, fd, make_string_literal(vm->m_heap, "/dev/stdin"), SCM_PORT_DIRECTION_IN, 0, SCM_PORT_BUFFER_MODE_BLOCK,
                           scm_false);
...
scm_obj_t subr_standard_output_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 0) {
    try {
      scm_port_t port;
      ...
      if (fd >= 0) fcntl(fd, F_SETFD, FD_CLOEXEC);
#endif
      port = make_std_port(vm->m_heap, fd, make_string_literal(vm->m_heap, "/dev/stdout"), SCM_PORT_DIRECTION_OUT, 0, SCM_PORT_BUFFER_MODE_BLOCK,
                           scm_false);
...
// standard-error-port
scm_obj_t subr_standard_error_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 0) {
    try {
      scm_port_t port;
      ...
      if (fd >= 0) fcntl(fd, F_SETFD, FD_CLOEXEC);
#endif
      port = make_std_port(vm->m_heap, fd, make_string_literal(vm->m_heap, "/dev/stderr"), SCM_PORT_DIRECTION_OUT, 0, SCM_PORT_BUFFER_MODE_NONE,
                           scm_false);
...
scm_obj_t subr_native_transcoder_descriptor(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 0) {
...
scm_obj_t subr_port_transcoder_descriptor(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// port-device-subtype
scm_obj_t subr_port_device_subtype(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
scm_obj_t subr_extract_accumulated_bytevector(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
scm_obj_t subr_get_accumulated_bytevector(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
scm_obj_t subr_extract_accumulated_string(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
scm_obj_t subr_get_accumulated_string(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
scm_obj_t subr_make_string_output_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 0) {
...
scm_obj_t subr_make_string_input_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (STRINGP(argv[0])) {
      scm_bvector_t transcoder = make_bvector(vm->m_heap, 3);
      transcoder->elts[0] = SCM_PORT_CODEC_UTF8;
...
// open-port
scm_obj_t subr_open_port(VM* vm, int argc, scm_obj_t argv[]) {
  scm_symbol_t proc;
  if (SYMBOLP(argv[0])) {
  ...
  switch (type) {
    case SCM_PORT_TYPE_NAMED_FILE: {
      if (STRINGP(name)) {
        ...
        if (file_options &
            ~(SCM_PORT_FILE_OPTION_NONE | SCM_PORT_FILE_OPTION_NO_CREATE | SCM_PORT_FILE_OPTION_NO_FAIL | SCM_PORT_FILE_OPTION_NO_TRUNCATE)) {
...
scm_obj_t subr_open_script_input_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (STRINGP(argv[0])) {
...
scm_obj_t subr_make_file_input_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (STRINGP(argv[0])) {
      scm_port_t port = NULL;
      try {
...
scm_obj_t subr_make_file_output_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (STRINGP(argv[0])) {
      scm_port_t port = NULL;
      try {
        port = make_file_port(vm->m_heap, (scm_string_t)argv[0], SCM_PORT_DIRECTION_OUT, SCM_PORT_FILE_OPTION_NO_FAIL,
                              SCM_PORT_BUFFER_MODE_BLOCK, scm_true);
        assert(PORTP(port));
        return port;
      } catch (io_exception_t& e) {
...
scm_obj_t subr_make_temporary_file_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 2) {
    if (STRINGP(argv[0])) {
...
// nonblock-byte-ready?
scm_obj_t subr_nonblock_byte_ready_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// get-char
scm_obj_t subr_get_char(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// lookahead-char
scm_obj_t subr_lookahead_char(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
scm_obj_t subr_port_has_port_position_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// port-position
scm_obj_t subr_port_position(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
scm_obj_t subr_port_has_set_port_position_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// set-port-position!
scm_obj_t subr_set_port_position(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 2) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// port-eof?
scm_obj_t subr_port_eof_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// get-u8
scm_obj_t subr_get_u8(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// lookahead-u8
scm_obj_t subr_lookahead_u8(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// get-byte
scm_obj_t subr_get_byte(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// lookahead-byte
scm_obj_t subr_lookahead_byte(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// get-bytevector-n
scm_obj_t subr_get_bytevector_n(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 2) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// get-bytevector-n!
scm_obj_t subr_get_bytevector_n_ex(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 4) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// get-bytevector-some
scm_obj_t subr_get_bytevector_some(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
...
// get-bytevector-all
scm_obj_t subr_get_bytevector_all(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock1(port->lock);
...
// get-string-n
scm_obj_t subr_get_string_n(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 2) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// get-string-n!
scm_obj_t subr_get_string_n_ex(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 4) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// get-string-all
scm_obj_t subr_get_string_all(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock1(port->lock);
...
// get-line
scm_obj_t subr_get_line(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock1(port->lock);
...
// get-datum
scm_obj_t subr_get_datum(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// put-u8
scm_obj_t subr_put_u8(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 2) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// put-byte
scm_obj_t subr_put_byte(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 2) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// put-bytevector
scm_obj_t subr_put_bytevector(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc >= 2 && argc <= 4) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// put-char
scm_obj_t subr_put_char(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 2) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// put-string
scm_obj_t subr_put_string(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc >= 2 && argc <= 4) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// put-datum
scm_obj_t subr_put_datum(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 2) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
...
// put-fasl
scm_obj_t subr_put_fasl(VM* vm, int argc, scm_obj_t argv[]) {
#if DISABLE_FASL
...
// write
scm_obj_t subr_write(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1 || argc == 2) {
    scm_port_t port;
    if (argc == 1) {
...
// display
scm_obj_t subr_display(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1 || argc == 2) {
    scm_port_t port;
    if (argc == 1) {
      port = vm->m_current_output;
...
// newline
scm_obj_t subr_newline(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 0 || argc == 1) {
    scm_port_t port;
    if (argc == 0) {
      port = vm->m_current_output;
...
// read-char
scm_obj_t subr_read_char(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 0 || argc == 1) {
    scm_port_t port;
    if (argc == 0) {
...
// peek-char
scm_obj_t subr_peek_char(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 0 || argc == 1) {
    scm_port_t port;
    if (argc == 0) {
      port = vm->m_current_input;
...
// write-char
scm_obj_t subr_write_char(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1 || argc == 2) {
    scm_port_t port;
    if (argc == 1) {
      port = vm->m_current_output;
...
// read
scm_obj_t subr_read(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 0 || argc == 1) {
    scm_port_t port;
    if (argc == 0) {
      port = vm->m_current_input;
...
scm_obj_t subr_make_transcoded_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 2) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
      ...
      if (BVECTORP(argv[1])) {
        ...
        try {
          scm_port_t textual =
              make_transcoded_port(vm->m_heap, make_list(vm->m_heap, 2, make_symbol(vm->m_heap, "transcoded"), port->name), port, transcoder);
          return textual;
        } catch (io_exception_t& e) {
...
scm_obj_t subr_set_current_input_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
...
scm_obj_t subr_set_current_output_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
...
scm_obj_t subr_set_current_error_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
...
scm_obj_t subr_shutdown_output_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (PORTP(argv[0])) {
      scm_port_t port = (scm_port_t)argv[0];
      scoped_lock lock(port->lock);
