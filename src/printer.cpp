...
class r6rs_param_t {
 ...
 public:
  r6rs_param_t(printer_t* printer, bool mode) {
    m_printer = printer;
    m_save = printer->r6rs(mode);
  }
...
printer_t::printer_t(VM* vm, scm_port_t port) {
  ...
  m_flush = false;
  m_r6rs = (FIXNUM(vm->m_flags.lexical_syntax_version) == 6);
}
...
void printer_t::flush() { port_flush_output(m_port); }

void printer_t::column_limit(int limit) { m_column_limit = limit; }

bool printer_t::symbol_need_bar(const uint8_t* s, int n) {
...
void printer_t::write_string(const uint8_t* utf8, int n) {
  uint32_t ucs4;
  int i = 0;
  while (i < n) {
    if (utf8[i] < 128) {
...
void printer_t::write_pretty_symbol(const uint8_t* utf8, int n) {
...
void printer_t::write_r6rs_symbol(const uint8_t* utf8, int n) {
  uint32_t cp;
  int i = 0;
  while (i < n) {
...
void printer_t::byte(uint8_t c) { port_put_byte(m_port, c); }

void printer_t::ucs4(uint32_t c) { write_ucs4(c); }

void printer_t::puts(const char* s) { port_puts(m_port, s); }
...
void printer_t::format_va_list(const char* fmt, va_list ap) {
  char buf[32];
  char c;
  while ((c = *fmt++) != 0) {
    switch (c) {
      case '~':
        ...
        switch (tolower(c)) {
          ...
          case 'm': {  // macro form
            ...
            m_radix = 10;
            scm_obj_t expr = va_arg(ap, scm_obj_t);
            if (cyclic_objectp(m_vm->m_heap, expr))
              write_shared(expr);
            else
              write(expr);
        ...
        switch (c) {
          case 'U': {
            ...
            if (ucs4 < 128) {
              ...
              switch (ucs4) {
                case 0:
                  port_puts(m_port, "nul(U+0000)");
                  break;
                case 7:
                  port_puts(m_port, "alarm(U+0007)");
                  break;
                case 8:
                  port_puts(m_port, "backspace(U+0008)");
                  break;
                case 9:
                  port_puts(m_port, "tab(U+0009)");
                  break;
                case 10:
                  port_puts(m_port, "linefeed(U+000A)");
                  break;
                case 11:
                  port_puts(m_port, "vtab(U+000B)");
                  break;
                case 12:
                  port_puts(m_port, "page(U+000C)");
                  break;
                case 13:
                  port_puts(m_port, "return(U+000D)");
                  break;
                case 27:
                  port_puts(m_port, "esc(U+001B)");
                  break;
                case 32:
                  port_puts(m_port, "space(U+0020)");
                  break;
                case 127:
                  port_puts(m_port, "delete(U+007F)");
                  break;
                default:
                  if (ucs4 < 32) {
...
bool printer_t::write_abbreviated(scm_obj_t obj) {
  assert(obj);
  if (SYMBOLP(obj)) {
...
void printer_t::write_ucs4(uint32_t c) {
  char utf8[4];
  int n = cnvt_ucs4_to_utf8(c, (uint8_t*)utf8);
  for (int i = 0; i < n; i++) port_put_byte(m_port, utf8[i]);
}

void printer_t::scan(scm_hashtable_t ht, scm_obj_t obj) {
...
static const char* proc_name(scm_obj_t obj) {
  if (SUBRP(obj)) {
...
void printer_t::write(scm_obj_t ht, scm_obj_t obj) {
  char buf[32];
  if (HASHTABLEP(ht)) {
  ...
  if (!CELLP(obj)) {
    ...
    if (CHARP(obj)) {
      ...
      if (m_escape) {
        ...
        switch (c) {
          case 0:
            port_puts(m_port, "nul");
            return;
          case 7:
            port_puts(m_port, "alarm");
            return;
          case 8:
            port_puts(m_port, "backspace");
            return;
          case 9:
            port_puts(m_port, "tab");
            return;
          case 10:
            port_puts(m_port, "linefeed");
            return;
          case 11:
            port_puts(m_port, "vtab");
            return;
          case 12:
            port_puts(m_port, "page");
            return;
          case 13:
            port_puts(m_port, "return");
            return;
          case 27:
            port_puts(m_port, "esc");
            return;
          case 32:
            port_puts(m_port, "space");
            return;
          case 127:
            port_puts(m_port, "delete");
            return;
        }
        if (c < 32) {
          snprintf(buf, sizeof(buf), "x%X", c);
    ...
    if (obj == scm_eof) {
      port_puts(m_port, "#<eof>");
      return;
    }
    if (obj == scm_timeout) {
      port_puts(m_port, "#<queue timeout>");
      return;
    }
    if (obj == scm_shutdown) {
      port_puts(m_port, "#<queue shutdown>");
      return;
    }
    if (obj == scm_true) {
      port_puts(m_port, "#t");
    ...
    if (obj == scm_undef) {
      port_puts(m_port, "#<undefined>");
      return;
    }
    if (obj == scm_unspecified) {
      port_puts(m_port, "#<unspecified>");
      return;
    }
    if (obj == scm_proc_apply) {
      port_puts(m_port, "#<procedure apply>");
      return;
    }
    if (obj == scm_proc_callcc) {
      port_puts(m_port, "#<procedure call/cc>");
      return;
    }
    if (obj == scm_proc_apply_values) {
      port_puts(m_port, "#<procedure apply-values>");
      return;
    }
  ...
  switch (tc) {
    ...
    case TC_SYMBOL: {
      ...
      if (m_unwrap) {
        ...
        if (UNINTERNEDSYMBOLP(symbol)) {
          ...
          if (m_escape)
            e = s + HDR_SYMBOL_SIZE(symbol->hdr);
          else
            e = s + strlen(s);
        }
        const char* p = strchr(s, IDENTIFIER_LIBRARY_SUFFIX);
        ...
        if (m_r6rs)
          write_r6rs_symbol((const uint8_t*)s, HDR_SYMBOL_SIZE(symbol->hdr));
        else
          write_pretty_symbol((const uint8_t*)s, HDR_SYMBOL_SIZE(symbol->hdr));
      } else {
        port_puts(m_port, s);
      }
      return;
    ...
    case TC_HASHTABLE: {
      ...
      switch (ht->type) {
        ...
        case SCM_HASHTABLE_TYPE_GENERIC: {
          ...
          if (hash_name)
            ...
            format("0x%x ", vector->elts[SCM_HASHTABLE_HANDLER_HASH]);
          const char* equiv_name = proc_name(vector->elts[SCM_HASHTABLE_HANDLER_EQUIV]);
          if (equiv_name)
            format("%s>", equiv_name);
          else
            format("0x%x>", vector->elts[SCM_HASHTABLE_HANDLER_EQUIV]);
          return;
        }
        default:
          assert(false);
      ...
      if (ht_datum)
        format(" %d/%d/%d>", ht_datum->live, ht_datum->used, ht_datum->capacity);
      else
        port_put_byte(m_port, '>');
      return;
    }
    ...
    case TC_SOCKET: {
      ...
      switch (socket->mode) {
        case SCM_SOCKET_MODE_CLIENT:
          port_puts(m_port, "#<client-socket");
          break;
        case SCM_SOCKET_MODE_SERVER:
          port_puts(m_port, "#<server-socket");
          break;
        default:
          port_puts(m_port, "#<socket");
          break;
      }
      ...
      switch (socket->socktype) {
        case SOCK_STREAM:
          port_puts(m_port, " stream");
          break;
        case SOCK_DGRAM:
          port_puts(m_port, " dgram");
          break;
        case SOCK_RAW:
          port_puts(m_port, " raw");
          break;
        default:
          format(" type(%d)", socket->socktype);
          break;
      }
      ...
      if (port->transcoder != scm_false) {
        switch (port->codec) {
          case SCM_PORT_CODEC_LATIN1:
            port_puts(m_port, " latin-1");
            break;
          case SCM_PORT_CODEC_UTF8:
            port_puts(m_port, " utf-8");
            break;
          case SCM_PORT_CODEC_UTF16:
            port_puts(m_port, " utf-16");
            break;
        }
      }
    ...
    case TC_CLOSURE: {
      ...
      if (closure->code == NULL) {
        if (closure->doc == scm_nil)
          format("#<closure 0x%x>", closure);
        else
          format("#<closure ~s>", closure->doc);
      } else {
        if (closure->doc == scm_nil)
          format("#<closure* 0x%x>", closure);
        else
      ...
      if (env == NULL) {
        if (closure->doc == scm_nil)
          format("#<closure 0x%x [0x%x]>", closure->pc, closure->code);
        else
          format("#<closure ~s [0x%x]>", closure->doc, closure->code);
      } else {
        if (closure->doc == scm_nil)
          format("#<closure 0x%x [0x%x] env:0x%x count:%d up:0x%x>", closure->pc, closure->code, env, env->count, env->up);
        else
          format("#<closure ~s [0x%x] env:0x%x count:%d up:0x%x>", closure->doc, closure->code, env, env->count, env->up);
      }
#endif
      return;
    }
...
void printer_t::write_shared(scm_obj_t obj) {
...
void printer_t::write(scm_obj_t obj) { write(scm_false, obj); }
