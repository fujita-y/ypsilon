...
static inline int cnvt_hex_char_to_int(int c) {
  if ((c >= '0') & (c <= '9'))
    return c - '0';
  else if ((c >= 'a') & (c <= 'f'))
    return c - 'a' + 10;
...
void reader_t::lexical_error(const char* fmt, ...) {
...
void reader_t::unget_ucs4() { m_ungetbuf_valid = true; }

int reader_t::lookahead_ucs4() {
  if (m_ungetbuf_valid) {
    if (m_ungetbuf == scm_eof) return EOF;
    return CHAR(m_ungetbuf);
  } else {
...
scm_obj_t reader_t::cons(scm_obj_t e1, scm_obj_t e2) { return make_pair(m_vm->m_heap, e1, e2); }

scm_obj_t reader_t::list2(scm_obj_t e1, scm_obj_t e2) { return cons(e1, cons(e2, scm_nil)); }

scm_obj_t reader_t::reverse_list(scm_obj_t lst, scm_obj_t tail) {
  scm_obj_t r = tail;
  while (PAIRP(lst)) {
    r = cons(CAR(lst), r);
    lst = CDR(lst);
...
scm_obj_t reader_t::skip_srfi30() {
  int c1;
  int c2;
  int nest = 0;
...
int reader_t::ensure_ucs4(int c) {
  assert(c >= 0);
...
int reader_t::read_hex_scalar_value() {
  int ucs4 = 0;
  int c = get_ucs4();
...
scm_obj_t reader_t::read_bytevector() {
...
scm_obj_t reader_t::read_number() {
  char buf[4096];
  read_thing(buf, sizeof(buf));
...
scm_obj_t reader_t::read_prefixed_number(int exactness, int radix, bool swap) {
  char buf[4096];
  read_thing(buf, sizeof(buf));
...
int reader_t::read_radix(int exactness) {
  if (get_ucs4() != '#') {
    unget_ucs4();
    return 10;
  }
...
int reader_t::read_exactness(int radix) {
  if (get_ucs4() != '#') {
    unget_ucs4();
    return 0;
  }
...
scm_obj_t reader_t::read_char() {
  static const struct {
    const char* name;
    int code;
  } char_name[] = {{"nul", 0x0000},      {"null", 0x0000},    {"alarm", 0x0007}, {"backspace", 0x0008}, {"tab", 0x0009},
                   {"linefeed", 0x000A}, {"newline", 0x000A}, {"vtab", 0x000B},  {"page", 0x000C},      {"return", 0x000D},
                   {"esc", 0x001B},      {"escape", 0x001B},  {"space", 0x0020}, {"delete", 0x007F}};
  int c = get_ucs4();
  if (c == 'x') {
    c = lookahead_ucs4();
  ...
  if (m_foldcase) {
    while (buf[i] != 0) {
      foldcase[i] = isascii(buf[i]) ? tolower(buf[i]) : buf[i];
      i++;
    }
  }
  foldcase[i] = 0;
...
int reader_t::read_escape_sequence() {
  int c = get_ucs4();
  switch (c) {
    case 'x':
      c = get_ucs4();
...
scm_obj_t reader_t::read_string() {
...
scm_obj_t reader_t::read_quoted_symbol() {
  if (FIXNUM(m_vm->m_flags.lexical_syntax_version) == 6) {
...
scm_obj_t reader_t::read_symbol() {
  char buf[MAX_READ_SYMBOL_LENGTH];
  int i = 0;
  while (i + 4 < array_sizeof(buf)) {
    int c = lookahead_ucs4();
    ...
    if (m_foldcase && isascii(c)) c = tolower(c);
    if (FIXNUM(m_vm->m_flags.lexical_syntax_version) > 6) {
      if (SYMBOL_CHARP(c)) {
        buf[i++] = c;
        continue;
...
static scm_obj_t encode_source_comment(int line, int column, bool file) {
  int comment = line * MAX_SOURCE_COLUMN + column;
  if (!file) comment = -comment;
  return MAKEFIXNUM(comment);
}

scm_obj_t reader_t::read_list(bool bracketed, bool vector) {
  ...
  switch (c) {
    ...
    case '#':
      ...
      switch (c) {
        ...
        case '(':
          return make_vector(m_vm->m_heap, read_list(false, true));
        case '|':
          return skip_srfi30();
        case '\\':
          return read_char();
        ...
        case 'b':
        case 'B':
          return read_prefixed_number(read_exactness(c), c, true);
        case 'o':
        case 'O':
          return read_prefixed_number(read_exactness(c), c, true);
        case 'd':
        ...
        case 'i':
        case 'I':
          return read_prefixed_number(c, read_radix(c), false);
        case 'e':
        case 'E':
          return read_prefixed_number(c, read_radix(c), false);
        case '\'':
...
void reader_t::put_note(scm_obj_t key, scm_obj_t value) {
  assert(m_note);
...
void reader_t::put_note(const char* symbol_name, scm_obj_t value) { put_note(make_symbol(m_vm->m_heap, symbol_name), value); }

void reader_t::parsing_range(int from, int to) {
  m_parsing_line_from = from;
  m_parsing_line_to = to;
}

void reader_t::parsing_line(int line) { parsing_range(line, line); }

scm_obj_t reader_t::lookup_graph(scm_tuple_t tuple) {
  scm_obj_t obj;
  scoped_lock lock(m_graph->lock);
...
void reader_t::link_graph(scm_obj_t obj) {
  if (PAIRP(obj)) {
    if (TUPLEP(CAR(obj))) {
...
scm_obj_t reader_t::read_graph(scm_hashtable_t note) {
  m_graph = make_hashtable(m_vm->m_heap, SCM_HASHTABLE_TYPE_EQ, lookup_mutable_hashtable_size(0));
  return read(note);
}
