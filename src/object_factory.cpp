...
scm_symbol_t make_symbol(object_heap_t* heap, const char* name) { return make_symbol(heap, name, strlen(name)); }
...
scm_symbol_t make_symbol_inherent(object_heap_t* heap, const char* name, int code) {
  ...
  int len = strlen(name);
  obj->hdr = scm_hdr_symbol | MAKEBITS(len, HDR_SYMBOL_SIZE_SHIFT) | MAKEBITS(code, HDR_SYMBOL_CODE_SHIFT) | HDR_SYMBOL_INHERENT_BIT;
  obj->name = (char*)heap->allocate_private(len + 1);
...
scm_string_t make_string(object_heap_t* heap, const char* name) { return make_string(heap, name, strlen(name)); }
...
scm_string_t make_string_literal(object_heap_t* heap, const char* name) { return make_string_literal(heap, name, strlen(name)); }
...
scm_vector_t make_vector(object_heap_t* heap, int n, scm_obj_t elt) {
...
scm_cont_t make_cont(object_heap_t* heap, scm_obj_t rec, void* lnk) {
...
scm_closure_t make_closure(object_heap_t* heap, int argc, int rest, void* env, scm_obj_t pc, scm_obj_t doc) {
...
scm_closure_t make_closure(object_heap_t* heap, scm_hdr_t hdr, void* env, scm_obj_t pc, scm_obj_t doc) {
  scm_closure_t obj = (scm_closure_t)heap->allocate_collectible(sizeof(scm_closure_rec_t));
  obj->hdr = hdr;
  obj->env = env;
  obj->code = NULL;
...
scm_bignum_t make_bignum(object_heap_t* heap, int n) {
  ...
  if (bytes <= INTERNAL_PRIVATE_THRESHOLD) {
    ...
    if (n)
      obj->elts = (digit_t*)((uintptr_t)obj + sizeof(scm_bignum_rec_t));
    else
      obj->elts = NULL;
  } else {
    ...
    obj->hdr = scm_hdr_bignum | MAKEBITS(n, HDR_BIGNUM_COUNT_SHIFT);
    if (n)
      obj->elts = (digit_t*)heap->allocate_private(sizeof(digit_t) * n);
    else
      obj->elts = NULL;
  }
  return obj;
...
scm_tuple_t make_tuple(object_heap_t* heap, int n, scm_obj_t elt) {
...
void rehash_hashtable(object_heap_t* heap, scm_hashtable_t ht, int nsize) {
...
void rehash_weakhashtable(object_heap_t* heap, scm_weakhashtable_t ht, int nsize) {
