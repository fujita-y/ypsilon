...
// make-socket
scm_obj_t subr_make_socket(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 6) {
...
// socket-shutdown
scm_obj_t subr_socket_shutdown(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 2) {
    if (SOCKETP(argv[0])) {
      if (FIXNUMP(argv[1])) {
...
// socket-close
scm_obj_t subr_socket_close(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (SOCKETP(argv[0])) {
      try {
...
// socket-send
scm_obj_t subr_socket_send(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 3) {
    if (SOCKETP(argv[0])) {
      if (BVECTORP(argv[1])) {
        int m_flags;
...
// socket-recv
scm_obj_t subr_socket_recv(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 3) {
    if (SOCKETP(argv[0])) {
      int len;
      int m_flags;
...
// socket-port
scm_obj_t subr_socket_port(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (SOCKETP(argv[0])) {
      scm_socket_t socket = (scm_socket_t)argv[0];
      if (socket->fd != INVALID_SOCKET) {
...
// socket-accept
scm_obj_t subr_socket_accept(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (SOCKETP(argv[0])) {
      scm_socket_t socket = (scm_socket_t)argv[0];
      if (socket->fd != INVALID_SOCKET) {
...
// socket?
scm_obj_t subr_socket_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
