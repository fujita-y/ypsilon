...
inline bool ucs4_whitespace(int c)  // http://www.unicode.org/Public/UNIDATA/PropList.txt
{
...
inline bool ucs4_constituent(uint32_t ucs4) {
  int offset = ucs4 / 8;
  int bit = 1 << (ucs4 & 7);
  return (s_constituent[offset] & bit) != 0;
}

inline bool ucs4_subsequent(uint32_t ucs4) {
  int offset = ucs4 / 8;
  int bit = 1 << (ucs4 & 7);
