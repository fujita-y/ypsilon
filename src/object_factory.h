...
scm_closure_t make_closure(object_heap_t* heap, scm_hdr_t hdr, void* env, scm_obj_t code, scm_obj_t doc);
...
inline void* new_heapenv_rec(object_heap_t* heap, size_t size) {
...
inline void* new_heapcont_rec(object_heap_t* heap, size_t size) {
  scm_hdr_t* hdr = (scm_hdr_t*)heap->allocate_collectible(sizeof(scm_hdr_t) + size);
  *hdr = scm_hdr_heapcont | ((uintptr_t)size << HDR_HEAPCONT_SIZE_SHIFT);
  return hdr + 1;
}

inline scm_pair_t make_pair(object_heap_t* heap, scm_obj_t car, scm_obj_t cdr) {
  VERIFY_DATUM(car);
  VERIFY_DATUM(cdr);
...
inline scm_pair_t make_immutable_pair(object_heap_t* heap, scm_obj_t car, scm_obj_t cdr) {
  VERIFY_DATUM(car);
  VERIFY_DATUM(cdr);
