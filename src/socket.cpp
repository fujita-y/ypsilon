...
static void throw_socket_error(int operation, int code) { throw io_exception_t(operation, code); }

static void throw_socket_error(int operation, const char* message) { throw io_exception_t(operation, message); }

void socket_open(scm_socket_t s, const char* node, const char* service, int family, int type, int flags, int protocol) {
...
scm_obj_t socket_name_string(object_heap_t* heap, scm_socket_t s) {
  ...
  if (s->family == AF_INET6)
    snprintf(nbuf, sizeof(nbuf), "[%s]:%s", hbuf, sbuf);
  else
    snprintf(nbuf, sizeof(nbuf), "%s:%s", hbuf, sbuf);
