...
static void throw_io_error(int operation, int code) { throw io_exception_t(operation, code); }

static void throw_io_error(int operation, const char* message) { throw io_exception_t(operation, message); }

static inline int io_open(const char* path, int oflag, int pmode) {
#if USE_CLOEXEC && defined(O_CLOEXEC)
...
static inline int io_close(fd_t fd) { return close(fd); }

static inline int io_read(fd_t fd, void* buf, size_t nbytes) { return read(fd, buf, nbytes); }

static inline int io_write(fd_t fd, void* buf, size_t nbytes) { return write(fd, buf, nbytes); }

static inline int io_pread(fd_t fd, void* buf, size_t nbytes, off64_t offset) { return pread(fd, buf, nbytes, offset); }

static inline int io_pwrite(fd_t fd, void* buf, size_t nbytes, off64_t offset) { return pwrite(fd, buf, nbytes, offset); }

static inline off64_t io_lseek64(fd_t fd, off64_t offset, int origin) { return lseek(fd, offset, origin); }

static inline fd_t io_mkstemp(char* tmpl) { return mkstemp(tmpl); }
...
static bool no_input_buffered(scm_port_t port) {
  return ((port->lookahead_size == 0) && (port->buf == NULL || port->buf_head == port->buf_tail || port->buf_state != SCM_PORT_BUF_STATE_READ));
}

static bool no_output_buffered(scm_port_t port) {
  return (port->buf_state != SCM_PORT_BUF_STATE_WRITE || port->buf == NULL || port->buf_head == port->buf_tail);
}

static void throw_codec_error(int operation, const char* message, scm_obj_t ch) { throw io_codec_exception_t(operation, message, ch); }

static ssize_t device_read(scm_port_t port, uint8_t* p, int size, off64_t mark) {
...
static void device_close(scm_port_t port) {
  if (port->type == SCM_PORT_TYPE_NAMED_FILE) {
...
static void init_port_buffer(scm_port_t port) {
...
static void init_port_transcoder(scm_port_t port) {
  ...
  if (BVECTORP(transcoder)) {
    ...
    if ((port->codec == SCM_PORT_CODEC_NATIVE) && (port->eol_style == SCM_PORT_EOL_STYLE_NATIVE) &&
...
static void init_port_tracking(scm_port_t port) {
  port->mark = 0;
  port->line = 1;
  port->column = 1;
...
void port_open_bytevector(scm_port_t port, scm_obj_t name, int direction, scm_obj_t bytes, scm_obj_t transcoder) {
  ...
  port->subtype = SCM_PORT_SUBTYPE_NONE;
  port->handlers = scm_false;
  if (port->direction & SCM_PORT_DIRECTION_OUT)
    port->bytes = scm_false;
  else
    port->bytes = bytes;
  port->name = name;
...
static void port_update_line_column(scm_port_t port, int c) {
  assert(PORTP(port));
  port->lock.verify_locked();
  if (c == '\n') {
    port->line++;
...
int port_get_bytes(scm_port_t port, uint8_t* p, int bsize) {
  assert(PORTP(port));
  if (bsize == 0) return 0;
  port->lock.verify_locked();
  assert(port->direction & SCM_PORT_DIRECTION_IN);
          ...
          case SCM_PORT_TYPE_NAMED_FILE: {
            if (port->buf) {
              ...
              while (port->lookahead_size < code_length) {
                int n = device_read(port, port->lookahead + port->lookahead_size, code_length - port->lookahead_size,
...
static bool bad_leading_utf16(uint32_t ucs2) { return (ucs2 == 0xFFFE) || (ucs2 == 0xFFFF) || ((ucs2 >= 0xDC00) & (ucs2 <= 0xDFFF)); }

static bool bad_secondary_utf16(uint32_t ucs2) { return !((ucs2 >= 0xDC00) & (ucs2 <= 0xDFFF)); }

static bool bad_ucs4(uint32_t ucs4) {
  return (ucs4 > 0x10FFFF) || (ucs4 == 0xFFFE) || (ucs4 == 0xFFFF) || ((ucs4 >= 0xD800) & (ucs4 <= 0xDFFF));
}

static bool decode_surrogate_pair(uint32_t* ucs4, uint32_t right) {
...
static void encode_surrogate_pair(uint32_t ucs4, uint32_t* left, uint32_t* right) {
  uint32_t n = ucs4 - 0x10000;
  *left = (n >> 10) + 0xD800;
  *right = (n & 0x3FF) + 0xDC00;
}

static bool test_bom(scm_port_t port, const uint8_t utf16[2]) {
...
static bool test_bom(scm_port_t port, const int utf16[2]) {
  if (utf16[0] == 0xFE && utf16[1] == 0xFF) {
    port->bom_be = true;
    return true;
  } else if (utf16[0] == 0xFF && utf16[1] == 0xFE) {
...
static uint32_t cnvt_utf16_to_ucs4_using_bom(scm_port_t port, const uint8_t utf16[2]) {
  if (port->bom_le) return ((uint32_t)utf16[1] << 8) + utf16[0];
  return ((uint32_t)utf16[0] << 8) + utf16[1];
}

static uint32_t cnvt_utf16_to_ucs4_using_bom(scm_port_t port, const int utf16[2]) {
  if (port->bom_le) return (uint32_t)((utf16[1] << 8) + utf16[0]);
  return (uint32_t)((utf16[0] << 8) + utf16[1]);
}

scm_obj_t port_lookahead_utf16(scm_port_t port) {
  assert(PORTP(port));
  port->lock.verify_locked();
  assert(port->direction & SCM_PORT_DIRECTION_IN);
  if (port->opened) {
...
scm_obj_t port_get_utf16(scm_port_t port) {
  assert(PORTP(port));
  port->lock.verify_locked();
  assert(port->direction & SCM_PORT_DIRECTION_IN);
  if (port->opened) {
...
void port_set_mark(scm_port_t port, off64_t offset) {
  assert(PORTP(port));
  port->lock.verify_locked();
  if (port->opened) {
    port_flush_output(port);
...
static void port_put_utf8(scm_port_t port, int32_t ucs4) {
  assert(PORTP(port));
  uint8_t utf8[4];
...
static void port_put_utf16(scm_port_t port, int32_t ucs4) {
  assert(PORTP(port));
  if (ucs4 >= 0x10000) {
    uint32_t left;
    uint32_t right;
...
scm_string_t port_get_string(object_heap_t* heap, scm_port_t port) {
  ...
  scm_string_t string;
  if (port->buf == NULL)
    string = make_string(heap, "");
  else
    string = make_string(heap, (const char*)port->buf_head, port->buf_tail - port->buf_head);
  return string;
}
...
static scm_obj_t port_lookahead_ch(scm_port_t port) {
  switch (port->codec) {
...
static scm_obj_t port_get_ch(scm_port_t port) {
  switch (port->codec) {
    case SCM_PORT_CODEC_UTF8:
...
void port_set_port_position(scm_port_t port, off64_t off) { port_set_mark(port, off); }

int port_output_buffer_mode(scm_port_t port) { return port->buffer_mode; }

static void port_putc(scm_port_t port, int32_t ucs4) {
  switch (port->codec) {
    case SCM_PORT_CODEC_UTF8: {
