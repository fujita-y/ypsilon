...
static void raise_implementation_restriction_violation(VM* vm, scm_obj_t who, scm_string_t message, scm_obj_t irritant) {
  assert(who);
  assert(message);
...
void letrec_violation(VM* vm) {
  ...
  if (vm->m_flags.warning_level == scm_false) {
    vm->apply_scheme(
        proc, 2, scm_false,
        make_string_literal(vm->m_heap,
                            "binding construct attempt to reference uninitialized variable, use '--warning' to perform expansion time check"));
  } else {
    vm->apply_scheme(proc, 2, scm_false,
                     make_string_literal(vm->m_heap, "binding construct attempt to reference uninitialized variable, check warning messages"));
  }
}
...
void wrong_type_argument_violation(VM* vm, const char* who, int position, const char* expected, scm_obj_t got, int argc, scm_obj_t argv[]) {
  ...
  if (argc < 2) {
    if (got == NULL)
      prt.format("expected %s, but missing", expected);
    else
      prt.format("expected %s, but got ~r", expected, got);
  } else {
    if (got == NULL)
      prt.format("expected %s, but missing for argument %d", expected, got, position + 1);
    else
      prt.format("expected %s, but got ~r, as argument %d", expected, got, position + 1);
  }
...
void raise_error(VM* vm, const char* who, const char* description, int code) {
  ...
  printer_t prt(vm, port);
  if (code)
    prt.format("%s (%d)", description, code);
  else
    prt.format("%s", description);
  scm_string_t message = port_extract_string(vm->m_heap, port);
...
void thread_global_access_violation(VM* vm, scm_obj_t name, scm_obj_t value) {
  vm->backtrace_seek();
  raise_assertion_violation(vm, scm_false, make_string(vm->m_heap, "child thread attempt to modify global variable"),
...
void thread_lexical_access_violation(VM* vm, scm_obj_t name, scm_obj_t value) {
  ...
  raise_assertion_violation(vm, scm_false, make_string(vm->m_heap, "child thread attempt to modify immutable parent variable"), irritants);
}
...
void thread_unsupported_operation_violation(VM* vm, const char* subr, int argc, scm_obj_t argv[]) {
  ...
  int last = argc;
  while (--last >= 0) irritants = make_pair(vm->m_heap, argv[last], irritants);
  raise_assertion_violation(vm, make_string(vm->m_heap, subr), make_string(vm->m_heap, "invalid use in child thread"),
...
void thread_object_access_violation(VM* vm, const char* subr, int argc, scm_obj_t argv[]) {
  ...
  int last = argc;
  while (--last >= 0) irritants = make_pair(vm->m_heap, argv[last], irritants);
  raise_assertion_violation(vm, scm_false, make_string(vm->m_heap, "child thread attempt to modify parent object"),
                            make_pair(vm->m_heap, vm->m_heap->lookup_system_environment(make_symbol(vm->m_heap, subr)), irritants));
}
...
void serialize_cyclic_object_violation(VM* vm, const char* who, scm_obj_t obj, int argc, scm_obj_t argv[]) {
  ...
  scm_port_t port = make_bytevector_port(vm->m_heap, make_symbol(vm->m_heap, "string"), SCM_PORT_DIRECTION_OUT, scm_false, scm_true);
  scoped_lock lock(port->lock);
  printer_t(vm, port).format("encountered non-serializable object, cyclic reference found while serializing ~r", obj);
  scm_string_t message = port_extract_string(vm->m_heap, port);
  if (argc < 2) {
    raise_assertion_violation(vm, make_symbol(vm->m_heap, who), message, NULL);
  } else {
