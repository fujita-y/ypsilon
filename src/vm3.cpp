...
void VM::apply_scheme(scm_obj_t proc, int argc, ...) {
...
void VM::apply_scheme_argv(scm_obj_t proc, int argc, scm_obj_t argv[]) {
  assert(SUBRP(proc) || CLOSUREP(proc));
  scm_obj_t code = LIST2(CONS(INST_LOAD_CONST, proc), LIST1(INST_APPLY));
  for (int i = 1; i <= argc; i++) code = CONS(CONS(INST_PUSH_CONST, argv[argc - i]), code);
  if (CDR(m_pc) == scm_nil) {
    code = CONS(scm_unspecified, code);
  } else {
    if (PAIRP(CAR(m_pc)) && CAAR(m_pc) == INST_PUSH_SUBR) {
...
scm_obj_t VM::call_scheme_stub(scm_obj_t proc, int argc, scm_obj_t argv[]) {
  assert(SUBRP(proc) || CLOSUREP(proc));
  scm_obj_t code = LIST2(CONS(INST_LOAD_CONST, proc), LIST1(INST_APPLY));
...
scm_obj_t VM::call_scheme_argv(scm_obj_t proc, int argc, scm_obj_t argv[]) {
  assert(SUBRP(proc) || CLOSUREP(proc));
...
scm_obj_t VM::call_scheme(scm_obj_t proc, int argc, ...) {
  assert(SUBRP(proc) || CLOSUREP(proc));
  scm_obj_t assistant = lookup_system_closure(".@apply-scheme-proc-assistant");
  scm_obj_t* param = (scm_obj_t*)alloca(sizeof(scm_obj_t) * (argc + 1));
  param[0] = proc;
...
void VM::check_vm_env(void* lnk) {
  ...
  if (!m_heap->m_concurrent_heap.in_heap(lnk)) {
  ...
  for (int i = 0; i < env->count; i++) {
    if (CELLP(top[i])) {
      if (!m_heap->m_concurrent_heap.is_collectible(top[i])) {
...
void VM::check_vm_cont(void* lnk) {
  ...
  if (!(STACKP(lnk) || m_heap->m_concurrent_heap.in_heap(lnk))) {
