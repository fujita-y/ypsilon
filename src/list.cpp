...
scm_obj_t list_tail(scm_obj_t lst, int n) {
  if (n < 0) return NULL;
  scm_obj_t obj = lst;
  while (--n >= 0) {
    if (PAIRP(obj))
...
static inline bool containerp(scm_obj_t obj) { return PAIRP(obj) || VECTORP(obj) || TUPLEP(obj); }
...
static bool cyclic_object_test(scm_obj_t lst, ancestor_t& ancestor, int depth) {
top:
  if (CELLP(lst)) {
