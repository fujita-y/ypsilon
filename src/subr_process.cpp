...
scm_obj_t subr_process_spawn(VM* vm, int argc, scm_obj_t argv[]) {
  ...
  if (argc >= 6) {
      ...
      if (PORTP(argv[2])) {
        ...
        switch (port->type) {
          ...
          case SCM_PORT_TYPE_SOCKET:
            fd0 = ((scm_socket_t)port_socket(port))->fd;
            break;
        }
      ...
      if (PORTP(argv[3])) {
        ...
        switch (port->type) {
          case SCM_PORT_TYPE_NAMED_FILE:
            ...
            break;
          case SCM_PORT_TYPE_SOCKET:
            fd1 = ((scm_socket_t)port_socket(port))->fd;
            break;
        }
      ...
      if (PORTP(argv[4])) {
        ...
        switch (port->type) {
          case SCM_PORT_TYPE_NAMED_FILE:
            ...
            break;
          case SCM_PORT_TYPE_SOCKET:
            fd2 = ((scm_socket_t)port_socket(port))->fd;
            break;
        }
    ...
    if (extra_fd) {
      ...
      if (fd0 != INVALID_FD)
        while (fd0 < 3)
          if ((fd0 = dup(fd0)) == -1) goto dup_fail;
      if (fd1 != INVALID_FD)
        while (fd1 < 3)
          if ((fd1 = dup(fd1)) == -1) goto dup_fail;
      if (fd2 != INVALID_FD)
        while (fd2 < 3)
          if ((fd2 = dup(fd2)) == -1) goto dup_fail;
    }
    sysfunc = "pipe";
    ...
    assert(sizeof(pid_t) == sizeof(int));
    return make_list(
        vm->m_heap, 4, int_to_integer(vm->m_heap, cpid),
        (pipe0[1] != INVALID_FD) ? make_std_port(vm->m_heap, pipe0[1], make_string_literal(vm->m_heap, "process-stdin"), SCM_PORT_DIRECTION_OUT,
                                                 SCM_PORT_FILE_OPTION_NONE, SCM_PORT_BUFFER_MODE_BLOCK, scm_false)
                                 : scm_false,
        (pipe1[0] != INVALID_FD) ? make_std_port(vm->m_heap, pipe1[0], make_string_literal(vm->m_heap, "process-stdout"), SCM_PORT_DIRECTION_IN,
                                                 SCM_PORT_FILE_OPTION_NONE, SCM_PORT_BUFFER_MODE_BLOCK, scm_false)
                                 : scm_false,
        (pipe2[0] != INVALID_FD) ? make_std_port(vm->m_heap, pipe2[0], make_string_literal(vm->m_heap, "process-stderr"), SCM_PORT_DIRECTION_IN,
                                                 SCM_PORT_FILE_OPTION_NONE, SCM_PORT_BUFFER_MODE_BLOCK, scm_false)
                                 : scm_false);
  }
...
// process-wait
scm_obj_t subr_process_wait(VM* vm, int argc, scm_obj_t argv[]) {
  int option = 0;
  if (argc == 2) {
...
scm_obj_t subr_lookup_process_environment(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (STRINGP(argv[0])) {
...
scm_obj_t subr_process_environment_alist(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 0) {
    extern char** environ;
    char** p = environ;
    ...
    while (*p) {
      ...
      if (e) {
        obj =
            make_pair(vm->m_heap, make_pair(vm->m_heap, make_string_literal(vm->m_heap, s, e - s), make_string_literal(vm->m_heap, e + 1)), obj);
      }
      p++;
    }
    return obj;
...
// getenv
scm_obj_t subr_getenv(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
...
// system
scm_obj_t subr_system(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc >= 1) {
    if (STRINGP(argv[0])) {
      scm_string_t string = (scm_string_t)argv[0];
...
// errno/string
scm_obj_t subr_errno_string(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 0) {
