...
void object_heap_t::init_heap(size_t pool_size, size_t init_size) {
  ...
  m_concurrent_heap.set_finalize_proc([this](void* obj) { ::finalize(this, obj); });
...
#endif
  // cache configuration
  int base_cache_limit = m_collect_trip_bytes / SLAB_SIZE;
  m_cons.m_cache_limit = base_cache_limit;
  m_flonums.m_cache_limit = base_cache_limit / 2;
  m_weakmappings.m_cache_limit = base_cache_limit / 8;
#if USE_CONST_LITERAL
...
scm_obj_t object_heap_t::lookup_system_environment(scm_symbol_t symbol) {
...
void object_heap_t::intern_system_environment(scm_symbol_t symbol, scm_obj_t value) {
...
void object_heap_t::intern_system_subr(const char* name, subr_proc_t proc) {
  ...
  while (count) {
    ...
    if (WEAKMAPPINGP(obj)) {
      if (traits->cache->state(obj)) {
        ...
        if (CELLP(key) && SLAB_TRAITS_OF(key)->cache->state(key) == false) wp->key = wp->value = scm_false;
      }
      count--;
    }
    p += size;
...
static void accumulate_object_count(void* obj, int size, void* refcon) {
...
void object_heap_t::display_heap_statistics(scm_port_t port) {
  int n_free = 0;
  int n_general = 0;
  int n_slab = 0;
  int n_gcslab = 0;
    ...
    switch (m_concurrent_pool.m_pool[n]) {
      ...
      case PTAG_USED | PTAG_SLAB | PTAG_GC:
        ...
        if (traits->refc == 0) {
          ...
          if (traits->free) {
            if (traits->cache == &m_immutable_cons)
              port_put_byte(port, 'l');
            else
              port_put_byte(port, 'o');
          } else {
            if (traits->cache == &m_immutable_cons)
              port_put_byte(port, 'L');
            else
...
void object_heap_t::init_inherents() {
...
static const char* verify_interior_obj(void* ref, object_heap_t* heap) {
  if (ref == NULL) return NULL;
  return verify_obj(SLAB_TRAITS_OF(ref)->cache->lookup(ref), heap);
}

static void check_collectible(void* obj, int size, void* refcon) {
...
void object_heap_t::validate_concurrent_slab(void* slab) {
  concurrent_slab_t* ca = SLAB_TRAITS_OF(slab)->cache;
  bool hit = false;
...
void object_heap_t::init_architecture_feature() {
...
#define ARCH_CCONST(name) put_hashtable(m_architecture_feature, make_symbol(this, #name), MAKEFIXNUM(name))
  ...
  #define ARCH_CFALSE(name)                                                       \
    do {                                                                          \
      printf("ARCH_CFALSE(%s)\n", #name);                                         \
      put_hashtable(m_architecture_feature, make_symbol(this, #name), scm_false); \
    } while (0)
#endif
#ifdef AF_UNSPEC
