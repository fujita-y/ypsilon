...
// fixnum?
scm_obj_t subr_fixnum_pred(VM* vm, int argc, scm_obj_t argv[]) {
...
// fixnum-width
scm_obj_t subr_fixnum_width(VM* vm, int argc, scm_obj_t argv[]) {
...
// least-fixnum
scm_obj_t subr_least_fixnum(VM* vm, int argc, scm_obj_t argv[]) {
...
// greatest-fixnum
scm_obj_t subr_greatest_fixnum(VM* vm, int argc, scm_obj_t argv[]) {
...
// fx=?
scm_obj_t subr_fx_eq(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) return (argv[0] == argv[1]) ? scm_true : scm_false;
    bad = FIXNUMP(argv[0]) ? 1 : 0;
    goto raise_bad;
  }
  if (argc == 1) {
...
// fx<?
scm_obj_t subr_fx_lt(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
...
// fx>?
scm_obj_t subr_fx_gt(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) return ((intptr_t)argv[0] > (intptr_t)argv[1]) ? scm_true : scm_false;
    bad = FIXNUMP(argv[0]) ? 1 : 0;
...
// fx<=?
scm_obj_t subr_fx_le(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) return ((intptr_t)argv[0] <= (intptr_t)argv[1]) ? scm_true : scm_false;
    bad = FIXNUMP(argv[0]) ? 1 : 0;
...
// fx>=?
scm_obj_t subr_fx_ge(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) return ((intptr_t)argv[0] >= (intptr_t)argv[1]) ? scm_true : scm_false;
    bad = FIXNUMP(argv[0]) ? 1 : 0;
...
// fxzero?
scm_obj_t subr_fx_zero_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
...
// fxpositive?
scm_obj_t subr_fx_positive_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
...
// fxnegative?
scm_obj_t subr_fx_negative_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
    if (FIXNUMP(argv[0])) return (FIXNUM(argv[0]) < 0) ? scm_true : scm_false;
...
// fxodd?
scm_obj_t subr_fx_odd_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
...
// fxeven?
scm_obj_t subr_fx_even_pred(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
...
// fxmax
scm_obj_t subr_fx_max(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
...
// fxmin
scm_obj_t subr_fx_min(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) return ((intptr_t)argv[0] < (intptr_t)argv[1]) ? argv[0] : argv[1];
    bad = FIXNUMP(argv[0]) ? 1 : 0;
...
// fx+
scm_obj_t subr_fx_add(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
...
// fx*
scm_obj_t subr_fx_mul(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) {
...
// fx-
scm_obj_t subr_fx_sub(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) {
      intptr_t n = FIXNUM(argv[0]) - FIXNUM(argv[1]);
...
// fxdiv
scm_obj_t subr_fx_div(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
...
// fxdiv0
scm_obj_t subr_fx_div0(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (argv[1] == MAKEFIXNUM(0)) {
    ...
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) {
      scm_obj_t obj = arith_integer_div0(vm->m_heap, argv[0], argv[1]);
      if (FIXNUMP(obj)) return obj;
      implementation_restriction_violation(vm, "fxdiv0", "return value out of fixnum range", obj, argc, argv);
      return scm_undef;
    }
    bad = FIXNUMP(argv[0]) ? 1 : 0;
    goto raise_bad;
...
// fxnot
scm_obj_t subr_fx_not(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
...
// fxand
scm_obj_t subr_fx_and(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
...
// fxior
scm_obj_t subr_fx_ior(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) return MAKEFIXNUM(FIXNUM(argv[0]) | FIXNUM(argv[1]));
    bad = FIXNUMP(argv[0]) ? 1 : 0;
...
// fxxor
scm_obj_t subr_fx_xor(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) return MAKEFIXNUM(FIXNUM(argv[0]) ^ FIXNUM(argv[1]));
    bad = FIXNUMP(argv[0]) ? 1 : 0;
...
// fxif
scm_obj_t subr_fx_if(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 3) {
    ...
    if (!FIXNUMP(argv[0])) bad = 0;
    goto raise_bad;
    if (!FIXNUMP(argv[1])) bad = 1;
    goto raise_bad;
    if (!FIXNUMP(argv[2])) bad = 2;
    goto raise_bad;
  }
...
// fxbit-count
scm_obj_t subr_fx_bit_count(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
...
// fxlength
scm_obj_t subr_fx_length(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
...
// fxfirst-bit-set
scm_obj_t subr_fx_first_bit_set(VM* vm, int argc, scm_obj_t argv[]) {
  if (argc == 1) {
...
// fxbit-set?
scm_obj_t subr_fx_bit_set_pred(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) {
      intptr_t fx1 = FIXNUM(argv[0]);
...
// fxcopy-bit
scm_obj_t subr_fx_copy_bit(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 3) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1]) & FIXNUMP(argv[1])) {
      intptr_t fx1 = FIXNUM(argv[0]);
...
// fxarithmetic-shift
scm_obj_t subr_fx_arithmetic_shift(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) {
      intptr_t fx1 = FIXNUM(argv[0]);
      ...
      if (fx2 > -FIXNUM_BITS && fx2 < FIXNUM_BITS) {
        ...
        implementation_restriction_violation(vm, "fxarithmetic-shift", "return value out of fixnum range",
                                             arith_logash(vm->m_heap, argv[0], argv[1]), argc, argv);
        return scm_undef;
      }
...
scm_obj_t subr_fx_arithmetic_shift_left(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) {
      intptr_t fx1 = FIXNUM(argv[0]);
      ...
      if (fx2 >= 0 && fx2 < FIXNUM_BITS) {
        intptr_t n = fx1 << fx2;
        if (((n >> fx2) == fx1) & (n >= FIXNUM_MIN) & (n <= FIXNUM_MAX)) return MAKEFIXNUM(n);
        implementation_restriction_violation(vm, "fxarithmetic-shift-left", "return value out of fixnum range",
                                             arith_logash(vm->m_heap, argv[0], argv[1]), argc, argv);
        return scm_undef;
      }
...
scm_obj_t subr_fx_arithmetic_shift_right(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 2) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1])) {
      intptr_t fx1 = FIXNUM(argv[0]);
      ...
      if (fx2 >= 0 && fx2 < FIXNUM_BITS) {
        intptr_t n = fx1 >> fx2;
        if ((n >= FIXNUM_MIN) & (n <= FIXNUM_MAX)) return MAKEFIXNUM(n);
        implementation_restriction_violation(vm, "fxarithmetic-shift-right", "return value out of fixnum range",
                                             intptr_to_integer(vm->m_heap, n), argc, argv);
        return scm_undef;
      }
...
// fxbit-field
scm_obj_t subr_fx_bit_field(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 3) {
    if (FIXNUMP(argv[0]) & FIXNUMP(argv[1]) & FIXNUMP(argv[1])) {
      intptr_t fx1 = FIXNUM(argv[0]);
...
// fxcopy-bit-field
scm_obj_t subr_fx_copy_bit_field(VM* vm, int argc, scm_obj_t argv[]) {
  int bad;
  if (argc == 4) {
