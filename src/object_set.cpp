...
void object_set_t::lock() { m_lock.lock(); }

void object_set_t::unlock() { m_lock.unlock(); }

void object_set_t::init(object_heap_t* heap) {
  m_heap = heap;
...
scm_obj_t object_set_t::get(const char* name, int len) {
  assert(m_heap);
...
void object_set_t::put(scm_obj_t obj) {
  assert(m_heap);
  m_lock.verify_locked();
