...
void serializer_t::emit_u8(uint8_t octet) {
  m_buf[m_buf_mark] = octet;
  m_buf_mark++;
  if (m_buf_mark + MAX_BUNDLE_SIZE >= m_buf_size) expand();
}

void serializer_t::emit_u32(uint32_t n) {
...
void serializer_t::emit_u64(uint64_t n) {
#if ARCH_LITTLE_ENDIAN
...
void serializer_t::emit_bytes(const uint8_t* s, int n) {
...
void serializer_t::push(scm_obj_t obj) {
...
void serializer_t::scan(scm_obj_t obj) {
loop:
  if (CELLP(obj)) {
    if (PAIRP(obj)) {
      scan(CAR(obj));
...
void serializer_t::put_list(scm_obj_t obj) {
  int count = 0;
  while (PAIRP(obj)) {
    push(CAR(obj));
    obj = CDR(obj);
...
void serializer_t::put_datum(scm_obj_t obj) {
  if (!CELLP(obj)) {
...
uint8_t deserializer_t::fetch_u8() { return *m_buf++; }

void deserializer_t::fetch_bytes(uint8_t* p, int n) {
  memcpy(p, m_buf, n);
  m_buf += n;
  if (m_buf > m_buf_tail) throw true;
}

uint32_t deserializer_t::fetch_u32() {
#if ARCH_LITTLE_ENDIAN
...
uint64_t deserializer_t::fetch_u64() {
#if ARCH_LITTLE_ENDIAN
...
scm_obj_t deserializer_t::get_datum() {
...
void deserializer_t::get_tagged() {
  int buflen = 128;
...
scm_obj_t deserializer_t::translate(scm_bvector_t obj) {
  try {
    m_buf = obj->elts;
