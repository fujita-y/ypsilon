...
(define-syntax ?ifeq?
  (syntax-rules ()
    ((_ (x . y) b kt kf) (??!apply kf #f))
    ((_ () b kt kf) (??!apply kf #f))
    ((_ a b _kt _kf)
     (let-syntax ((aux
                   (syntax-rules (a)
                     ((_ a kt kf) (??!apply kt #t))
                     ((_ other kt kf) (??!apply kf #f)))))
       (aux b _kt _kf)))))
                                   ...
                                   (?loop
                                    ...
                                    (??!lambda
                                      ...
                                      (?cons
                                       ...
                                       (??! g1035)
                                       _?kg1030)))))))))))))
       (?number-two (??!lambda (g1038) (?loop (??! g1038) _?n _?kg1029)))))))
       
(define-syntax ?sieve
  (syntax-rules ()
    ((_ _?lst _?kg1039)
     (letrec-syntax ((?choose-pivot
                       (syntax-rules ()
                         ((_ _?lstg1041 _?kg1040)
                          (?ifnull?
                            _?lstg1041
                     ...
                     (?do-sieve
                       (syntax-rules ()
                         ((_ _?stepg1059 _?currentg1060 _?lstg1061 _?kg1058)
                          (?ifnull?
                            _?lstg1061
                            (??!lambda (g1062) (??!apply _?kg1058 _?lstg1061))
                            (??!lambda
                              (g1063)
                              (?ifnumber-zero?
                                _?currentg1060
...
(define ans)
(?is-prime (((((()))))) (??!lambda (x) (set! ans (quote (??! x)))))
(format #t "=> ~s" ans)
(if (eq? ans 'prime)
	  (format #t "~&Passed~%")
    (begin
			(format #t "~&Failure: expected prime but got ~s~%" ans)
			(exit #f)))
