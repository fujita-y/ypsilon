...
(test-begin "literal pattern match")
(test-eval! (define-syntax foo1 (syntax-rules (L M) ((foo L ...) 1))))
(test-equal (foo1) => 1)
(test-equal (foo1 L) => 1)
(test-syntax-violation (foo1 M))
(test-syntax-violation (foo1 N))
(test-equal (foo1 L L) => 1)
(test-syntax-violation (foo1 L M))
(test-syntax-violation (foo1 L N))
(test-eval! (define-syntax foo2 (syntax-rules (L M) ((foo L ... T) (list 1 'T)))))
(test-syntax-violation (foo2))
(test-equal (foo2 L) => (1 L))
(test-equal (foo2 M) => (1 M))
(test-equal (foo2 N) => (1 N))
(test-equal (foo2 L L) => (1 L))
(test-equal (foo2 L M) => (1 M))
(test-equal (foo2 L N) => (1 N))
(test-equal (foo2 L L O) => (1 O))
...
(test-begin "nested escape ellipsis")
(test-eval! (define-syntax be-like-begin-synrule
              (syntax-rules ()
                ((_ name1)
                 (define-syntax name1
                   (syntax-rules ()
                     ((_ name2)
                      (define-syntax name2
                        (syntax-rules ()
                          ((_ e0 e1 (... (... ...)))
                           (begin e0 e1 (... (... ...)))))))))))))
(test-eval! (be-like-begin-synrule foo1))
(test-eval! (foo1 bar1))
(test-equal (bar1 1 2 3) => 3)
(test-eval! (define-syntax be-like-begin-syncase
              (lambda (x)
                (syntax-case x ()
                  ((_ name1)
                   #'(define-syntax name1
                            ...
                            #'(define-syntax name2
                                (lambda (x)
                                  (syntax-case x ()
                                    ((_ e0 e1 (... (... ...)))
                                     #'(begin e0 e1 (... (... ...))))))))))))))))
(test-eval! (be-like-begin-syncase foo2))
(test-eval! (foo2 bar2))
(test-equal (bar2 4 5 6) => 6)
(test-end)

(test-begin "destructuring")
(test-equal (let ((m 1) (o 100))
              (destructuring-match '(1 2 3)
                ((_ m n) (even? m) (+ m n o))
                (_ #f))) => 105)
(test-equal (let ((m 1) (o 100))
              (destructuring-match '(2 3 4)
                ((_ m n) (even? m) (+ m n o))
                (_ #f))) => #f)
(test-equal (let ((m 1) (o 100))
