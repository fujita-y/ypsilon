...
(define load
  (lambda (path . env)
    (or (null? env) (null? (cdr env)) (assertion-violation 'load "wrong number of arguments" (cons path env)))
    (or (null? env) (environment? (car env)) (eq? (tuple-ref (car env) 0) 'type:eval-environment)
        (assertion-violation 'load (format "expected environment, but got ~r, as argument 2" (car env))))
