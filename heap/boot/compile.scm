...
(define make-closure-comment
  (lambda (form)
    (cond ((and (current-closure-comments) (core-hashtable-ref (current-closure-comments) form #f)) => cdr)
          (else '()))))
...
(define ht-special-subr-expression
  (let ((ht (make-core-hashtable)))
    (for-each
      (lambda (x) (core-hashtable-set! ht x #t))
    ...
    (define compile-anonymous
      (lambda (form cte tail comment)
        (if tail
            ...
            `(,@(compile-argument-each (cdr form) cte)
              (subr.gloc.of ,(car form) ,(length (cdr form)) . ,comment)))))
        ...
        (if (core-hashtable-contains? ht-special-subr-expression subr)
            (cond ((null? (cdr form))
                  ...
                  ((null? (cdddr form))
                   (if tail
                       (cond ((eq? subr cons)
                              `(,@(compile-argument (cadr form) cte)
                                ,@(compile-expression (caddr form) cte #f #f)
                                ,(cons 'ret.cons comment)))
                             ((eq? subr eq?)
                              `(,@(compile-argument (cadr form) cte)
                                ,@(compile-expression (caddr form) cte #f #f)
                                ,(cons 'ret.eq? comment)))
                       ...
                       (cond ((assq subr alist-special-binary-subr)
                              (let ((opd1 (cadr form)) (opd2 (caddr form)))
                                (let-values (((lhs rhs inst)
                                              (cond ((and (fixnum? opd1) (symbol? opd2) (iloc? opd2 cte))
                                                     (values
                                                       opd2
                                                       opd1
                                                       (cdr (assq subr alist-special-binary-subr-negate))))
                                                    ((and (fixnum? opd2) (symbol? opd1) (iloc? opd1 cte))
                                                     (values opd1 opd2 (cdr (assq subr alist-special-binary-subr))))
                                                    (else (values #f #f #f)))))
                                  (cond (inst `((,inst ,(make-iloc-operand lhs cte) ,rhs . ,comment)))
                                        ((iloc? opd2 cte)
                                         `(,@(compile-expression opd1 cte #f #f)
                                           (,(cdr (assq subr alist-special-binary-subr-iloc))
                                            ,(make-iloc-operand opd2 cte) . ,comment)))
                                        ((iloc? opd1 cte)
                                         `(,@(compile-expression opd2 cte #f #f)
                                           (,(cdr (assq subr alist-special-binary-subr-iloc-negate))
                                            ,(make-iloc-operand opd1 cte) . ,comment)))
                                        (else (compile-anonymous form cte tail comment))))))
                             ((eq? subr +)
                              (let ((opd1 (cadr form)) (opd2 (caddr form)))
                                (let-values (((lhs rhs)
                  ...
                  ((null? (cdddr form))
                   (cond ((eq? subr cons)
                         ...
                         ((eq? subr +)
                          (let ((opd1 (cadr form)) (opd2 (caddr form)))
                            (let-values (((lhs rhs)
                                          (cond ((and (iloc? opd1 cte) (fixnum? opd2)) (values opd1 opd2))
...
(define compile-argument
  (lambda (form cte)
    (cond ((pair? form)
           (cond ((top-level-subr (car form)) (compile-subr-argument form cte))
                 ((eq? (car form) 'lambda)
                  (let ((code (compile-lambda-helper (cadr form) (cddr form) (make-closure-comment form) cte)))
                    (if (local-closure? form) (list (cons 'push.close+ code)) (list (cons 'push.close code)))))
                 ((eq? (car form) 'quote) `((push.const . ,(cadr form))))
                 (else `(,@(compile-expression form cte #f #f) ,(list 'push)))))
          ((symbol? form)
                   ...
                   (if (iloc? form cte)
                       (let ((opd (make-iloc-operand form cte)))
                         (case (car opd)
                               ...
                               (else `((push.iloc ,@opd)))))
                       `((push.gloc.of ,form))))))
          (else `((push.const . ,form))))))

(define compile-call
  (lambda (proc argc comment cte)
    (cond ((pair? proc) `(,@(compile-expression proc cte #f #f) ,(list 'apply)))
          ((symbol? proc)
           (cond ((iloc? proc cte)
                  (if (core-hashtable-contains? (ht-local-closures) proc)
                      `((apply.iloc+ ,(make-iloc-operand proc cte) . ,comment))
                      `((apply.iloc ,(make-iloc-operand proc cte) . ,comment))))
                 ((top-level-subr proc) `((ret.subr.gloc.of ,proc . ,comment)))
                 (else `((apply.gloc.of ,proc . ,comment)))))
          (else `((const . ,proc) (apply ,@comment))))))

(define compile-expression-begin
  (lambda (form cte discard tail)
    (let ((body (cdr form)))
      (cond ((null? body) (if tail (list (list 'ret.const.unspec)) '()))
            ((null? (cdr body)) (compile-expression (car body) cte discard tail))
            (else (compile-expression-seq body cte discard tail))))))
...
(define compile-expression-set!
  (lambda (form cte discard tail)
    (let ((body (compile-expression (caddr form) cte #f #f)) (retval (if tail (list (list 'ret.const.unspec)) '())))
...
(define compile-expression-let
  (lambda (form cte discard tail)
    (cond ((null? (cadr form)) (compile-expression-seq (cddr form) cte discard tail))
          (else
            (collect-local-closure (cadr form))
            (let ((vars (map car (cadr form))))
              (let ((code
                      `(,@(compile-argument-each (map cadr (cadr form)) cte)
                        (extend . ,(length vars))
             ...
             (let ((cte (cte-extend-iloc (list (car binding)) cte)))
               (cond ((and (pair? (cadr binding)) (eq? (caadr binding) 'lambda))
                      ...
                      (let ((init (cadr binding)))
                        (let ((init-code
                                (compile-lambda-helper (cadr init) (cddr init) (make-closure-comment init) cte))
                              (body-code (compile-expression-seq (cddr form) cte #f #t))
                              (inst (if (local-closure? init) 'extend.enclose+ 'extend.enclose)))
                          (let ((code `((,inst ,@init-code) ,@body-code))) (if tail code (list (cons 'call code)))))))
                     (else
                       (let ((code
                               `(,(cons 'extend.unbound 1)
              ...
              (let-values (((front back)
                            (let loop ((lst bindings) (front '()))
                              (if (null? lst)
                                  (values bindings '())
                                  (let ((init (cadar lst)))
                                    (if (if (pair? init) (memq (car init) ''lambda) (not (symbol? init)))
                ...
                (let ((cte (cte-extend-iloc (map car bindings) cte)))
                  (let ((front-code (compile-argument-each (map cadr front) cte))
                        (back-code
                          (apply
                            append
                            (map (lambda (iloc expr) `(,@expr (set.iloc ,iloc)))
                    ...
                    (let ((code
                            (if (null? front-code)
                                ...
                                `((extend.unbound . ,(length bindings))
                                  ,@front-code
                                  (enclose . ,(length front-code))
                                  ,@back-code
                                  ,@body-code))))
                      (if tail code (list (cons 'call code))))))))))))
  ...
  (lambda (form cte discard tail)
    ...
    (define compile-unary-special
      (lambda (form cte tail inst inst-ret-const)
        (let ((arg (cadadr form)) (true (caddr form)))
    ...
    (define compile-binary-special
      (lambda (form cte tail inst inst-ret-const)
        (let ((args (cdadr form)) (true (caddr form)))
          (let ((arg-code (append (compile-argument (car args) cte) (compile-expression (cadr args) cte #f #f)))
                (true-code (compile-expression true cte #f #t))
                (false-code (compile-expression (cadddr form) cte #f #t)))
            (if (and (null? (cdr true-code)) (memq (caar true-code) '(const ret.const)))
                (cond (tail `(,@arg-code (,inst-ret-const . ,(cdar true-code)) ,@false-code))
            ...
            (cond ((and (pair? (cadr form)) (top-level-subr (caadr form)))
                   ...
                   (lambda (subr)
                     (case (length (cadr form))
                           ((2)
                            (cond ((eq? subr null?) (compile-unary-special form cte tail 'if.null? 'if.null?.ret.const))
                                  ((eq? subr pair?) (compile-unary-special form cte tail 'if.pair? 'if.pair?.ret.const))
                                  ((eq? subr symbol?)
                                   (compile-unary-special form cte tail 'if.symbol? 'if.symbol?.ret.const))
                                  (else (compile-anonymous form cte discard tail))))
                           ((3)
                            (cond ((eq? subr eq?) (compile-binary-special form cte tail 'if.eq? 'if.eq?.ret.const))
    ...
    (define compile-clause
      (lambda (lst cte)
        ...
        (define compile-anonymous-negate
          (lambda (lst cte)
            `(,@(compile-expression (car lst) cte #f #f)
              ,(cons 'if.false.ret.const #t)
              ,@(compile-clause (cdr lst) cte))))
        (define compile-unary-special
          (lambda (lst cte inst)
            `(,@(compile-expression (cadar lst) cte #f #f) ,(cons inst #t) ,@(compile-clause (cdr lst) cte))))
        (define compile-binary-special
          (lambda (lst cte inst)
            `(,@(compile-argument (cadar lst) cte)
              ,@(compile-expression (caddar lst) cte #f #f)
              ,(cons inst #t)
              ,@(compile-clause (cdr lst) cte))))
        (if (null? (cdr lst))
            ...
            (cond ((and (pair? (car lst)) (top-level-subr (caar lst)))
                   ...
                   (lambda (subr)
                     (if (eq? subr not)
                                    ...
                                    (case (length (car lst))
                                          ((2)
                                           (cond ((eq? subr null?)
                                                 ...
                                                 (else (compile-anonymous-negate lst cte))))
                                          ((3)
                                           (cond ((eq? subr eq?) (compile-binary-special lst cte 'if.not.eq?.ret.const))
                                                 ((or (eq? subr eqv?) (eq? subr equal?))
                         ...
                         (case (length (car lst))
                               ((2)
                                (cond ((eq? subr null?) (compile-unary-special lst cte 'if.null?.ret.const))
                                      ((eq? subr pair?) (compile-unary-special lst cte 'if.pair?.ret.const))
                                      ((eq? subr symbol?) (compile-unary-special lst cte 'if.symbol?.ret.const))
                                      (else (compile-anonymous lst cte))))
                               ((3)
                                (cond ((eq? subr eq?) (compile-binary-special lst cte 'if.eq?.ret.const))
                                      ((or (eq? subr eqv?) (eq? subr equal?))
                                       (let ((opd1 (cadr (car lst))) (opd2 (caddr (car lst))))
                                         (if (or (immediate-literal? opd1) (immediate-literal? opd2))
                               ...
                               (else (compile-anonymous lst cte))))))
                  (else (compile-anonymous lst cte))))))
    (cond ((null? (cdr form)) (cond (discard '()) (tail (list (cons 'ret.const #f))) (else (list (cons 'const #f)))))
          ((null? (cddr form)) (compile-expression (cadr form) cte discard tail))
          ((and (not tail)
                (null? (cdddr form))
                (pair? (caddr form))
            ...
            (cond ((and (pair? (car lst)) (top-level-subr (caar lst)))
                   ...
                   (lambda (subr)
                     (if (eq? subr not)
                         ...
                         (case (length (car lst))
                               ((2)
                                (cond ((eq? subr null?) (compile-unary-special lst cte 'if.not.null?.ret.const))
                                      ((eq? subr pair?) (compile-unary-special lst cte 'if.not.pair?.ret.const))
                                      ((eq? subr symbol?) (compile-unary-special lst cte 'if.not.symbol?.ret.const))
                                      (else (compile-anonymous lst cte))))
                               ((3)
                                (cond ((eq? subr eq?) (compile-binary-special lst cte 'if.not.eq?.ret.const))
                                      ((or (eq? subr eqv?) (eq? subr equal?))
                               ...
                               (else (compile-anonymous lst cte))))))
                  (else (compile-anonymous lst cte))))))
    (cond ((null? (cdr form)) (cond (discard '()) (tail (list (cons 'ret.const #t))) (else (list (cons 'const #t)))))
          ((null? (cddr form)) (compile-expression (cadr form) cte discard tail))
          (else (let ((code (compile-clause (cdr form) cte))) (if tail code (list (cons 'call code))))))))
...
(define compile-expression-seq
  (lambda (form cte discard tail)
    (define reverse-append
      (lambda (lst)
        (let loop ((lst lst) (ans '())) (cond ((null? lst) ans) (else (loop (cdr lst) (append (car lst) ans)))))))
    (let loop ((lst form) (acc '()))
      (cond ((null? lst) (reverse-append acc))
            (else
              (let ((e (compile-expression (car lst) cte (or discard (pair? (cdr lst))) (and tail (null? (cdr lst))))))
                (loop (cdr lst) (cons e acc))))))))
  ...
  (lambda (form cte discard tail)
    (cond ((pair? form)
           (cond ((core-hashtable-ref ht-dispatch-expression (car form) #f)
                 ...
                 ((top-level-subr (car form)) (compile-subr-expression form cte discard tail))
                 (else
                   (let ((call (compile-call (car form) (length (cdr form)) (make-application-comment form) cte))
                     ...
                     (let ((code `(,@touch ,@args ,@call))) (if tail code (list (cons 'call code))))))))
          (else
            (if tail
                (cond ((eq? form '|.&UNDEF|) (list (list 'ret.const.undef)))
                      ((unspecified? form) (list (list 'ret.const.unspec)))
                      ((symbol? form)
                       (if (iloc? form cte) `((ret.iloc ,@(make-iloc-operand form cte))) `((ret.gloc.of ,form))))
                      (else `((ret.const . ,form))))
                (cond (discard '())
                      ((eq? form '|.&UNDEF|) (list (list 'const.undef)))
                      ((unspecified? form) (list (list 'const.unspec)))
                      ((symbol? form)
                       (if (iloc? form cte)
                           (let ((opd (make-iloc-operand form cte)))
                             (case (car opd)
