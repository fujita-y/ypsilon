...
(define run-scheme-session
  (lambda ()
    ...
    (define init-env-acc
      (lambda ()
        (cond
          ((lookup-process-environment "YPSILON_ACC")
           =>
           (lambda (path)
             (cond ((directory-exists? (expand-path path)) (auto-compile-cache (expand-path path)))
                   (else
                     (format
                       (current-error-port)
                       "** ERROR in environment variable 'YPSILON_ACC': directory ~s not exist~%"
                       path)
                     (auto-compile-cache #f))))))))
    (define init-env-sitelib
      (lambda ()
        (cond
          ((lookup-process-environment "YPSILON_SITELIB")
           =>
           (lambda (paths)
             (for-each
               (lambda (path)
                 (cond ((directory-exists? (expand-path path)) (add-library-path (expand-path path)))
                       (else
                         (format
                           (current-error-port)
                           "** ERROR in environment variable 'YPSILON_SITELIB': directory ~s not exist~%"
                           path))))
               (reverse (string-split paths #\:))))))))
    (define init-env-loadpath
      (lambda ()
        (cond
          ((lookup-process-environment "YPSILON_LOADPATH")
           =>
           (lambda (paths)
             (for-each
               (lambda (path)
                 (cond ((directory-exists? (expand-path path)) (add-load-path (expand-path path)))
                       (else
                         (format
                           (current-error-port)
                           "** ERROR in environment variable 'YPSILON_LOADPATH': directory ~s not exist~%"
                           path))))
               (reverse (string-split paths #\:))))))))
    ...
    (define show-info
      (lambda ()
        (show-banner)
        (cond ((lookup-process-environment "YPSILON_ACC") => (lambda (path) (format #t ";; YPSILON_ACC=~a~%" path)))
