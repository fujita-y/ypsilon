...
(define-syntax time
  (syntax-rules ()
    ((_ expr)
     (let ((start (times)) (internal-start (get-internal-real-time)))
       (let ((result (apply (lambda () expr) '())))
         (let ((end (times)) (internal-end (get-internal-real-time)))
           (let ((real (* (- (vector-ref end 0) (vector-ref start 0)) 0.000000001))
             ...
             (format output-port "\t~s~%" (/ (exact->inexact (- internal-end internal-start)) internal-time-units-per-second))))
         result)))))
